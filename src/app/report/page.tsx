...
export default function ReportPage() {
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      ...
      // Keep UX consistent even when generation fails.
  ...
  return (
    <div className="report-page d-flex flex-column gap-4" data-testid="report-page">
      ...
      <Card className="p-4 report-section">
        <h2 className="cupertino-title" style={{ fontSize: "1.2rem" }}>{strings.report.coverTitle}</h2>
        <div style={{ fontSize: "0.9rem" }}>
          ...
          {selectedDocument && (
            <div>
              <strong>{strings.report.metadata}:</strong> tipo {selectedDocument.type}, plataforma {selectedDocument.platform}, idioma {selectedDocument.language}
            </div>
          )}
        </div>
      </Card>
      ...
      <Accordion title={strings.report.appendix} defaultOpen>
        <div className="cupertino-card-inset p-3 mb-3" style={{ fontSize: "0.84rem" }}>
          <h3 className="fw-semibold" style={{ fontSize: "0.95rem" }}>Categoria {"->"} Ícone {"->"} Justificativa</h3>
          <ul className="mb-0">
            {SEMIOTIC_MAP.map((item) => (
              <li key={item.category}><strong>{CATEGORY_LABELS[item.category]}</strong> | ícone {item.icon_id} | {item.significance}</li>
            ))}
          </ul>
        </div>

        <div className="cupertino-card-inset p-3 mb-3" style={{ fontSize: "0.84rem" }}>
          <h3 className="fw-semibold" style={{ fontSize: "0.95rem" }}>Impacto {"->"} Interpretação</h3>
          <ul className="mb-0">
        ...
        <div>
          <h3 className="fw-semibold" style={{ fontSize: "0.95rem" }}>{strings.report.documentMapping}</h3>
          {semioForDoc ? (
